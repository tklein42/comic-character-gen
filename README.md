...
The final observation that I noticed when generating images using the IP-Adapter, the background in the images may be limited due to the same reason as previously mentioned. Since both outfit images have plain background, and the necessary `ip-adapter-scale` required to apply the outfit to the character is relatively high, it results in the model taking too much unnecessary information from the background in the outfit image. This can be evident especially in set 1 with outfit 1 where the backgrounds lack high quality details that are ideal for this project.
